...
$records_per_page = 50; // Set to 50 records per page for verified students
...
if ($search !== '') {
    ...
    $count_query = "SELECT COUNT(*) as total FROM `registration_26to27` WHERE isVerified ='1' AND (`name` LIKE ? OR `fname` LIKE ? OR `cnic` LIKE ? OR `id` LIKE ? OR `uhs_application_id` LIKE ?)";
    ...
    $var = "SELECT * FROM `registration_26to27` WHERE isVerified ='1' AND (`name` LIKE ? OR `fname` LIKE ? OR `cnic` LIKE ? OR `id` LIKE ? OR `uhs_application_id` LIKE ?) LIMIT ?, ?";
    ...
    $count_query = "SELECT COUNT(*) as total FROM `registration_26to27` WHERE isVerified ='1'";
    ...
    $var = "SELECT * FROM `registration_26to27` WHERE isVerified ='1' LIMIT ?, ?";
    if ($stmt = mysqli_prepare($conn, $var)) {
...
<div id="admin-content" class="animate-fade-in">
    ...
    <div class="topbar">
        <div class="topbar-title-section">
            <h1 class="topbar-title">Verified Students</h1>
            <p class="text-muted small mb-0">Browse and manage verified student admission profiles</p>
        </div>
        
        <!-- Live Search Box -->
        <form method="GET" action="verified.php" class="search-container">
            ...
            <?php if ($search !== ''): ?>
                <a href="verified.php" class="position-absolute end-0 top-50 translate-middle-y me-2 text-muted" title="Clear Search">
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <h5 class="admin-card-title">Verified Registration Records</h5>
                ...
                <tbody>
                    ...
                    if ($connect && mysqli_num_rows($connect) > 0) {
                        ...
                        while ($row = mysqli_fetch_array($connect)) {
                            ...
                            echo '<td>';
                            if ($row['isVerified'] == '1') {
                                echo '<span class="badge-custom badge-success-light"><i class="fa-solid fa-circle-check"></i> Verified</span>';
                            } else {
                                echo '<span class="badge-custom badge-danger-light"><i class="fa-solid fa-clock"></i> Unverified</span>';
                            }
                            echo '</td>';
                        ...
                        echo '<tr><td colspan="20" class="text-center py-4 text-muted"><i class="fa-solid fa-triangle-exclamation"></i> No verified records found.</td></tr>';
                    }
                    ?>
                </tbody>
            </table>
                ...
                <nav>
                    <ul class="pagination pagination-sm mb-0">
                        ...
                        <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                            <a class="page-link" href="verified.php?page=<?php echo $page - 1; ?><?php echo ($search !== '') ? '&search=' . urlencode($search) : ''; ?>">Previous</a>
                        </li>
                        ...
                        if ($start_page > 1) {
                            echo '<li class="page-item"><a class="page-link" href="verified.php?page=1' . (($search !== '') ? '&search=' . urlencode($search) : '') . '">1</a></li>';
                            if ($start_page > 2) {
                        ...
                        for ($i = $start_page; $i <= $end_page; $i++) {
                            echo '<li class="page-item ' . ($page == $i ? 'active' : '') . '"><a class="page-link" href="verified.php?page=' . $i . (($search !== '') ? '&search=' . urlencode($search) : '') . '">' . $i . '</a></li>';
                        }
                        ...
                        if ($end_page < $total_pages) {
                            ...
                            echo '<li class="page-item"><a class="page-link" href="verified.php?page=' . $total_pages . (($search !== '') ? '&search=' . urlencode($search) : '') . '">' . $total_pages . '</a></li>';
                        }
                        ?>
                        
                        <!-- Next Page -->
                        <li class="page-item <?php echo ($page >= $total_pages) ? 'disabled' : ''; ?>">
                            <a class="page-link" href="verified.php?page=<?php echo $page + 1; ?><?php echo ($search !== '') ? '&search=' . urlencode($search) : ''; ?>">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
